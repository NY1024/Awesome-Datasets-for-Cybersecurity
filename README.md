# Awesome-Datasets-for-Cybersecurity
![](1.png)
## Categories

- [Network](#network)
- [Website](#website)
- [URL](#url)

- [Fraud](#fraud)

- [Spam](#spam)

- [Threat Intelligence](#threat-intelligence)

- [Software](#software)

- [Forensics](#forensics)

- [Authentication](#authentication)

- [Payloads](#payloads)

- [Honeypot](#honeypot)

- [System](#system)

- [Misinformation](#misinformation)

- [Password](#password)

- [Comprehensive dataset](#comprehensive-dataset)

- [Benchmark](#benchmark)



## Network

**[1]MACCDC2012**



https://www.netresec.com/?page=MACCDC



The U.S. National CyberWatch Mid-Atlantic Collegiate Cyber Defense Competition (MACCDC) is a premier event that offers college and university students a unique opportunity to test their cybersecurity skills in a competitive environment, focusing on the operational management and protection of network infrastructures while responding to real-world business challenges and defending against attacks from a volunteer Red Team.





**[2]Unified Host and Network Data Set**



https://csr.lanl.gov/data/2017/



The Unified Host and Network Dataset includes de-identified network and host events collected from the Los Alamos National Laboratory enterprise network over approximately 90 days, with logs from Windows-based enterprise computers and internal routers, allowing for integrated analysis while maintaining security by matching anonymized identities across both data types, with some exceptions for well-known ports, system-level users, and redundant services.



**[3]The Malware Capture Facility Project**



https://mcfp.weebly.com/



The Malware Capture Facility Project by the Czech Technical University ATG Group aims to capture, analyze, and publish real and long-lived malware traffic, executing malware for extended periods, analyzing traffic manually and automatically, assigning ground-truth labels to various traffic types, and providing datasets to the community to aid in developing improved detection methods.



**[4]Cyber Defence Exercises**



http://www.netresec.com/?page=PcapFiles



This category includes network traffic from exercises and competitions, such as Cyber Defense Exercises (CDX) and red-team/blue-team competitions.



**[5]UNSW-NB15 Raw Network Packets**



https://www.unsw.adfa.edu.au/australian-centre-for-cyber-security/cybersecurity/ADFA-NB15-Datasets/



**[6]Malware traffic analysis**



https://www.malware-traffic-analysis.net/



A site for sharing packet capture (pcap) files and malware samples.



**[7]Network Forensics Puzzle Contest**



https://forensicscontest.com/puzzles



**[8]SampleCaptures**



https://wiki.wireshark.org/SampleCaptures#Sample_Captures



**[9]Public Data Sets**



https://redmine.openinfosecfoundation.org/projects/suricata/wiki/Public_Data_Sets



Collections of pcaps for testing, profiling.



**[10]LBNL-FTP-PKT**



https://ee.lbl.gov/anonymized-traces.html



The traces released here contain all incoming anonymous FTP connections (i.e. to port 21) to public FTP servers at the Lawrence Berkeley National Laboratory during a ten-day period.



**[11]Network Packet Dumps**



https://digitalcorpora.org/corpora/network-packet-dumps/



**[12]KDD Cup 1999 Data**



https://kdd.ics.uci.edu/databases/kddcup99/kddcup99.html



This is the data set used for The Third International Knowledge Discovery and Data Mining Tools Competition, which was held in conjunction with KDD-99 The Fifth International Conference on Knowledge Discovery and Data Mining.



**[13]Detecting Malicious URLs**



http://sysnet.ucsd.edu/projects/url/



The project is ongoing, with updates available on our site. You can download an anonymized ICML-09 dataset and sample code for the factor analysis approach from our AISTATS-10 paper.



**[14]OpenDNS Top Domains List**



https://github.com/opendns/public-domain-lists



The OpenDNS Top Domains List is the top 10,000 domain names our resolvers all over the globe are receiving queries for, sorted by popularity.



**[15]UDP**



https://github.com/rapid7/sonar/wiki/UDP



This data is gathered by sending protocol-specific UDP probes across the entire IPv4 address space. The types of probes sent each week continues to expand as the project matures.



**[16]DGA RCHIVE**



https://dgarchive.caad.fkie.fraunhofer.de/



DGArchive provides domain names that are algorithmically created by malware using Domain Generation Algorithms (DGAs).



**[17]IoT devices captures**



https://research.aalto.fi/en/datasets/iot-devices-captures



This dataset represents the traffic emitted during the setup of 31 smart home IoT devices of 27 different types (4 types are represented by 2 devices each). Each setup was repeated at least 20 times per device-type.



**[18]2017-SUEE-data-set**



https://github.com/vs-uulm/2017-SUEE-data-set



The data sets contain traffic in and out of the web server of the [Student Union for Electrical Engineering (Fachbereichsvertretung Elektrotechnik) at Ulm University](https://fs-et.de/).



**[19]HTTP DATASET CSIC 2010**



https://web.archive.org/web/20130924222653/http://iec.csic.es/dataset



**[20]MCFP**



https://mcfp.weebly.com/



The Malware Capture Facility Project is an effort from the Czech Technical University ATG Group for capturing, analyzing and publishing real and long-lived malware traffic.



## Website

**[1]Website Phishing**



https://archive.ics.uci.edu/dataset/379/website+phishing



The phishing problem is considered a vital issue in the e-commerce industry especially e-banking and e-commerce taking the number of online transactions involving payments. 



**[2]Phish-IRIS dataset**



https://web.cs.hacettepe.edu.tr/~selman/phish-iris-dataset/



Our dataset involves 1313 training and 1539 testing samples. The directory structure of the dataset has been formed in order to be used without an extra effort.



**[3]Website Classification Dataset**



https://data.webarchive.org.uk/opendata/ukwa.ds.1/classification/



The entire selective archive is manually curated, including classification of sites into a two-tiered subject hierarchy. We have made this manually-generated classification information available as an open dataset, in tab-separated column format.



## URL



## Fraud

**[1]Credit Card Fraud**



https://www.kaggle.com/code/samkirkiles/credit-card-fraud/data

## Spam

**[1]2007 TREC Public Spam Corpus**



https://plg.uwaterloo.ca/~gvcormac/treccorpus07/



This corpus may contain computer viruses, fraudulent solicitations, and other damaging material. The material should not be viewed with an ordinary file browser, email client, or web client.

## Threat Intelligence

**[1]MISP Threat Sharing**



https://www.misp-project.org/



**[2]malware-indicators**



https://github.com/citizenlab/malware-indicators



This repository includes all malware indicators that were found during the course of Citizen Lab investigations. Each directory corresponds to a single Citizen Lab report as seen below.



**[3] All**



https://otx.alienvault.com/browse/global/pulses?include_inactive=0&sort=-modified&page=1&limit=10



**[4]Malware Corpus Tracker**



http://tracker.h3x.eu/families



**[5]Cybercrime-track**



https://cybercrime-tracker.net/



**[6]Unit 42 iocs**



https://github.com/pan-unit42/iocs



This repository contains indicators related to Unit 42 Public Reports.



**[7]Threatfeeds**



https://threatfeeds.io/



**[8]C2IntelFeeds**



https://github.com/drb-ra/C2IntelFeeds



**[9]stopforumspam**



https://www.stopforumspam.com/downloads



**[10]The Majestic Million**



https://majestic.com/reports/majestic-million

## Software

**[1]C2**



https://github.com/misterch0c/what_is_this_c2







**[2]Malware Share**

https://virusshare.com/



http://www.malshare.com/



https://www.threatminer.org/



http://www.kernelmode.info/forum/viewforum.php?f=16



http://www.malwaregroup.com/



https://avcaesar.malware.lu/



http://oc.gtisc.gatech.edu:8080/



**[3]BODMAS Malware Dataset**



https://whyisyoung.github.io/BODMAS/



BODMAS is short for Blue Hexagon Open Dataset for Malware AnalysiS. We collaborate with Blue Hexagon to release a dataset containing timestamped malware samples and well-curated family information for research purposes. The BODMAS dataset contains 57,293 malware samples and 77,142 benign samples collected from August 2019 to September 2020, with carefully curated family information (581 families)



**[4]Malware Sample Sources**



https://github.com/Virus-Samples/Malware-Sample-Sources



This is a project created to make it easier for malware analysts to find virus samples for analysis, research, reverse engineering, or review.



**[5]Malware-Threat-Reports**



https://github.com/MalwareSamples/Malware-Feed



This repository contains actual malware, do not execute any of these files on your pc unless you know exactly what you are doing. They have been renamed for easier processing.



**[6]MalwareTrainingSets**



https://github.com/marcoramilli/MalwareTrainingSets



**[7]Elastic Malware Benchmark for Empowering Researchers**



https://github.com/elastic/ember



The EMBER dataset is a collection of features from PE files that serve as a benchmark dataset for researchers. 



**[8]Open Malware Database**



https://www.virussign.com/



Giant database dedicated to combating malware in the digital world



**[9]PlugX Chronicles**



https://rebsnippets.blogspot.com/2015/08/plugx-chronicles.html



This blog-post is to compile list of articles and information bits about the **PlugX** - malware used as a RAT by several APT groups.



**[10]The Drebin Dataset**



https://drebin.mlsec.org/



The dataset contains 5,560 applications from 179 different malware families. The samples have been collected in the period of August 2010 to October 2012 and were made available to us by the MobileSandbox project.



**[11]JavaScript Vulnerability DataSet**



http://www.inf.u-szeged.hu/~ferenc/papers/JSVulnerabilityDataSet/



**[12]payloads**



https://github.com/foospidy/payloads



**[13]Malware Knowledge Base**



https://owl.nchc.org.tw/

## Forensics

**[1]Digital Corpora**



http://digitalcorpora.org/



DigitalCorpora.org provides freely available digital corpora, including disk images, memory dumps, and network packet captures, for computer forensics education and research, with a special arrangement required for access to real-world data.



**[2]Verizon VERIS Database**



https://github.com/vz-risk/VCDB



The VERIS Community Database, supported by the Verizon RISK Team, aims to facilitate effective information sharing for collective intelligence in cybersecurity, despite challenges in sharing raw data due to partner agreements, and addresses the gap in comprehensive, publicly available datasets for security incident research and corporate decision-making.



**[3]Malware dump**



https://contagiodump.blogspot.com/





**[4]Windows EVTX Samples**



https://github.com/sbousseaden/EVTX-ATTACK-SAMPLES



This is a container for windows events samples associated to specific attack and post-exploitation techniques.



**[5]Aktaion2**



https://github.com/jzadeh/aktaion2/tree/master/data

## Authentication

**[1]User-Computer Authentication Associations in Time**



https://csr.lanl.gov/data/auth/



This anonymized dataset contains 708,304,516 successful authentication events collected over 9 continuous months from the Los Alamos National Laboratory (LANL) enterprise network, with each event recorded as "time,user,computer" in a comma-delimited format.



[2]



## Payloads

**[1]Web attack payloads**



https://github.com/foospidy/payloads



A collection of web attack payloads



[2]



## Honeypot

**[1]Sweetie data**



https://github.com/0xsha/sweetie-data



This repository contains three months of honeypot data, primarily collected using T-Pot, from December 2019 to February 2020, offering insights into malicious actor activities.



**[2]Threat_Research**



https://github.com/JonathanPhillips/Threat_Research



This is a centralized repository to dump threat research data gathered from my network of honeypots.



## System

**[1]Windows Audit Logs**



https://github.com/konstantinberlin/malware-windows-audit-log-detection



This is the github page for the AI-Sec 2015 publication "Malicious Behavior Detection using Windows Audit Logs" by Konstantin Berlin, David Slater, and Joshua Saxe.



**[2]Public Security Log Sharing Site**



https://log-sharing.dreamhosters.com/



This site contains various free shareable log samples from various systems, security and network devices, applications, etc. The logs are collected from real systems, some contain evidence of compromise and other malicious activity. 



**[3]self.logs**



http://www.secrepo.com/self.logs/

## Misinformation



**[1]Awesome Misinformation**



https://github.com/bodacea/misinfolinks



A curated list of awesome misinformation



[2]



## Password

**[1]yahoo_password_frequencies_corpus**



https://figshare.com/articles/dataset/Yahoo_Password_Frequency_Corpus/2057937



## Comprehensive dataset

**[1]Comprehensive, Multi-Source Cyber-Security Events Auth**



https://csr.lanl.gov/data/cyber1/



This dataset comprises 58 consecutive days of de-identified event data from Los Alamos National Laboratory’s internal network, including Windows authentication events, process start/stop events, DNS lookups, network flow data, and red teaming events, totaling 12GB compressed and 1.65 billion events for 12,425 users, 17,684 computers, and 62,974 processes, with all data uniformly de-identified and no external associations included.



**[2]Intelligence and Security Informatics Data Sets**



https://www.azsecure-data.org/get-data.html



The AZSecure-data PORTAL offers access to Web forums, phishing websites, Twitter data, and other datasets, with most available for direct download and some requiring a request to ailab@eller.arizona.edu, specifying the dataset and intended use, along with contact details. Dark Web forums collected up to 2012 support international Jihadi social media studies, and GeoWeb forums assess country risk, covering multiple languages and regions. The collection includes over 21M postings. Phishing websites from the University of Virginia are categorized and available for download, while Anti-Virus Security Software Tweets data contains over 700,000 tweets collected between 2008 and 2013. Other datasets are also accessible through the portal.



**[3]Security Datasets**



https://github.com/OTRF/Security-Datasets



The Security Datasets project is an open-source initiative providing both malicious and benign datasets from various platforms to the infosec community, facilitating data analysis and threat research.



**[4]Stanford Internet Research Data Repository**



https://scans.io/



The Stanford Internet Research Data Repository is a public archive of research datasets that describe the hosts, services, and websites on the Internet.



**[5]Canadian Institute for Cybersecurity datasets**



https://www.unb.ca/cic/datasets/index.html



Canadian Institute for Cybersecurity datasets are used around the world by universities, private industry, and independent researchers. 



**[6]publicDataset**



https://mcfp.felk.cvut.cz/publicDatasets/



**[7]awesome-industrial-control-system-security**



https://github.com/hslatman/awesome-industrial-control-system-security



A curated list of resources related to Industrial Control System (ICS) security.



**[8]DDS Dataset Collection**



https://datadrivensecurity.info/blog/pages/dds-dataset-collection.html



**[9]URLhaus API Documentation**



https://urlhaus-api.abuse.ch/



**[10]Datasets Overview**



https://www.stratosphereips.org/datasets-overview/





**[11]Azsecure-data**



https://www.azsecure-data.org/get-data.html



**[12]Cyber Security Datasets**



https://github.com/kdemertzis/Cyber-Security-Datasets

## Benchmark

**[1]AISecKG-Cybersecurity-Dataset**



https://github.com/garima0106/AISecKG-cybersecurity-dataset



**[2]SecEval**



https://github.com/XuanwuAI/SecEval



**[3]CyberQA**



https://github.com/priamai/CyberQA



**[4]CyberMetric**



https://github.com/CyberMetric

